 ...
 */
public class BeatBall1 extends View {
    private Path mPath;
    ...
    private int mballMaxy = -500;//球运动的最高点
    ...
    public BeatBall1(Context context) {
        super(context);
    }

    public BeatBall1(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initPaint();
        startAni();
    }
    ...
    public BeatBall1(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        canvas.drawCircle(30, height / 2+100, 25, mLineCirclePaint);//绳左球
        canvas.drawCircle(width - 30, height / 2+100, 25, mLineCirclePaint);//绳右球
        canvas.drawCircle(width / 2, height / 2+100 + mballControlly, 60, mballPaint);//画下落的球
        /*贝塞尔曲线绘制绳*/
        mPath.reset();
        mPath.moveTo(23, height / 2+100);
        mPath.quadTo(mLineControllx, height / 2+100 + mLineControlly, width - 23, height / 2+100);
